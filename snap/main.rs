...
fn main() -> Result<()> {
	...
	let config_path = args
		.path
		.or_else(|| env::var("TUWUNEL_CONFIG").ok().map(PathBuf::from))
		.unwrap_or_else(|| PathBuf::from("tuwunel.toml"));

	if config_path.exists() {
		println!(
			"Configuration file already exists at {}. Exiting gracefully.",
			config_path.display()
		);
		return Ok(());
	}
		...
		&& !parent.exists()
	{
		fs::create_dir_all(parent).map_err(|e| {
			anyhow::anyhow!(
				"Unable to write to {}, perhaps try sudo. ({})",
				config_path.display(),
				e
			)
		})?;
		...
		.with_validator(move |input: &str| {
			if input.trim().is_empty() {
				...
				Ok(Validation::Invalid(
					"Please enter a valid fully qualified domain name.".into(),
				))
			} else {
				Ok(Validation::Valid)
			}
	...
	let registration_token = Password::new("Registration token:")
		...
		.with_help_message(
			"A static registration token users will have to provide when creating an account.",
		)
		.with_validator(move |input: &str| {
			if input.trim().is_empty() {
			...
			if ent < 40.0 {
				Ok(Validation::Invalid(
					"Registration is insecure. Please choose a more secure token.".into(),
				))
			} else {
				Ok(Validation::Valid)
			}
	...
	println!(
		"Registration token entropy: {}",
		classify_entropy(estimate_entropy(registration_token))
	);

	let create_user_decision =
		Text::new("Would you like to create an initial admin user? [Yes/No/Quit]:")
	...
	let create_user = decision == "y" || decision == "yes" || decision.is_empty();
	...
	if create_user {
		...
		let admin_password = Password::new("Admin password:")
			...
			.with_validator(move |input: &str| {
				...
				if ent < 40.0 {
					Ok(Validation::Invalid(
						format!(
							"Admin password entropy is {}. Please choose a stronger password.",
							classify_entropy(ent)
						)
						.into(),
					))
				} else {
		...
		println!(
			...
			classify_entropy(estimate_entropy(&admin_password))
		);
	...
	// If running inside a Snap, we must change the default database path to the
	// writable SNAP_COMMON directory
	...
	if !create_user {
		...
		println!(
			"The daemon can now be started. If using snap, the service will pick up the config \
			 momentarily."
		);
		return Ok(());
	}
	...
	// We expect `tuwunel` binary to be situated in the exact same directory as this
	// tool.
	...
	let status = Command::new(&tuwunel_bin)
		...
		.arg(format!(
			"users create_user @{}:{} {}",
			admin_username, server_name, admin_password
		))
		.arg("--execute")
		.arg("server shutdown")
		.status()
		.with_context(|| {
			format!("Failed to evaluate tuwunel binary: {}", tuwunel_bin.display())
		})?;

	if status.success() {
		println!("\nInitialization Complete! \u{1F389}");
		println!(
			"The daemon can now be started, or will start automatically if you installed the \
			 Tuwunel snap."
		);
		println!(
			"It will by default listen only on 127.0.0.1:8008, you will probably want to direct \
			 traffic"
		);
