...
impl Retention {
    ...
    #[allow(dead_code)]
    pub(super) fn put_media_ref(&self, mxc: &str, mr: &MediaRef) { self.cf.raw_put(Self::key_mref(mxc), Cbor(mr)); }

    #[allow(dead_code)]
    pub(super) async fn get_media_event_ref(&self, event_id: &str, kind: &str) -> Result<Option<MediaEventRef>> {
    ...
    pub(super) fn insert_mxcs_on_event(&self, event_id: &str, room_id: &str, mxcs: &[(String, bool, String)]) {
        ...
        for (mxc, local, kind) in mxcs.iter() {
            ...
            let mer = MediaEventRef { mxc: mxc.clone(), room_id: room_id.to_owned(), kind: kind.clone() };
            let key_mer = Self::key_mer(event_id, kind).into_bytes();
            let val_mer = serialize_val(Cbor(&mer)).expect("serialize mer").to_vec();
            ...
            let (mr, new) = match current.and_then(|h| h.deserialized::<Cbor<MediaRef>>()) {
                ...
                _ => (MediaRef { refcount: 1, local: *local, first_seen_ts: now, last_seen_ts: now }, true),
            };
            if new {
            ...
            let val_mref = serialize_val(Cbor(&mr)).expect("serialize mref").to_vec();
    ...
    let mut stream = self.cf.stream_raw_prefix::<&str, Cbor<MediaEventRef>, _>(&prefixb);
        while let Some(item) = stream.next().await.transpose()? {
            ...
            if let Some(handle) = current {
                ...
                puts.push((key_mref.into_bytes(), val_mref));
                if should_queue {
                    let kq = Self::key_queue(&mer.mxc).into_bytes();
                    let vq = serialize_val(Cbor(&DeletionCandidate { mxc: mer.mxc.clone(), enqueued_ts: now_secs() }))?.to_vec();
                    puts.push((kq, vq));
                    warn!(%event_id, mxc = %mer.mxc, "retention: queued media for deletion");
                    to_delete.push(mer.mxc.clone());
                }
            }
    ...
    pub(super) fn queue_media_for_deletion(&self, mxc: &str) {
        let key = Self::key_queue(mxc);
        // overwrite / insert candidate with fresh timestamp
        let cand = DeletionCandidate { mxc: mxc.to_owned(), enqueued_ts: now_secs() };
        warn!(mxc, "retention: fallback queue media for deletion");
        self.cf.raw_put(key, Cbor(&cand));
    }

    /// worker: processes queued deletion candidates after grace period.
    pub(super) async fn worker_process_queue(&self, service: &Service, grace: Duration) -> Result<()> {
    let prefix = K_QUEUE.as_bytes();
    warn!(?grace, "retention: worker iteration start");
    let mut stream = self.cf.stream_raw_prefix::<&str, Cbor<DeletionCandidate>, _>(&prefix);
        let mut processed = 0usize;
        ...
        while let Some(item) = stream.next().await.transpose()? {
            ...
            let deleted_bytes = self.delete_local_media(service, &cand.mxc).await.unwrap_or(0);
            if deleted_bytes > 0 {
    ...
    async fn delete_local_media(&self, service: &Service, mxc: &str) -> Result<u64> {
        ...
        use ruma::Mxc;
        let mxc_parsed: Mxc<'_> = mxc.try_into().map_err(|_| err!(Request(BadJson("invalid mxc"))))?;

        // delete originals
        let keys = service.db.search_mxc_metadata_prefix(&mxc_parsed).await.unwrap_or_default();
        let mut total = 0u64;
        for key in keys {
