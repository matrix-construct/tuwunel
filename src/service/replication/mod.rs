...
use tuwunel_core::{Err, Result, err, error, expected, implement, info, warn};
			...
			if resume_seq == 0 {
				...
				if sidecar.exists() {
					...
					if db_seq > 0 {
						...
						warn!("Empty database, bootstrap required. Triggering restart.");
						return Err!(Database("Restarting for pre-open checkpoint bootstrap"));
					}
				}
			}
				...
				match self.run_stream(&primary_url).await {
					...
					| Err(ref e) if is_wal_gap_error(e) => {
						...
						if let Err(e) = self.server.shutdown() {
							...
							return Err!(Database("WAL gap; failed to trigger restart"));
						}
					...
					| Err(ref e) => {
						...
						error!(
							"Replication stream error: {e}; reconnecting in {backoff_ms}ms..."
						);
					},
				}
...
async fn run_stream(&self, primary_url: &str) -> Result {
	...
	if !resp.status().is_success() {
		return Err!(Database("Primary returned {} for WAL stream", resp.status()));
	}
	...
	while self.server.is_running() && !self.promoted.load(Ordering::Acquire) {
		tokio::select! {
			...
			chunk = byte_stream.next() => {
				...
				match chunk {
					Err(e) => return Err!(Database("WAL stream read: {e}")),
					Ok(chunk) => {
...
fn drain_frames(&self, buf: &mut Vec<u8>) -> Result {
	...
	while let Ok((frame, consumed)) = WalFrame::decode(&buf[offset..]) {
		...
		offset = expected!(offset + consumed);
	}
	buf.drain(..offset);
